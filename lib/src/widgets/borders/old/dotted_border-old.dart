...
class DottedShapeBorder extends AbstractBorder {
  const DottedShapeBorder({
    ...
    this.dashPattern = const <double>[1, 1],
  });
  ...
  void paintBorder(Canvas canvas, Rect rect, {TextDirection? textDirection}) {
    ...
    if (top.width > 0) {
      ...
      _drawDottedLine(canvas, Offset(tlX, rrect.top), Offset(trX, rrect.top),
          dashPattern, paint);
    }
    if (right.width > 0) {
      paint.color = right.color;
      paint.strokeWidth = right.width;
      _drawDottedLine(canvas, Offset(rrect.right, trY),
          Offset(rrect.right, brY), dashPattern, paint);
    }
    if (bottom.width > 0) {
      paint.color = bottom.color;
      paint.strokeWidth = bottom.width;
      _drawDottedLine(canvas, Offset(brX, rrect.bottom),
          Offset(blX, rrect.bottom), dashPattern, paint);
    }
    if (left.width > 0) {
      paint.color = left.color;
      paint.strokeWidth = left.width;
      _drawDottedLine(canvas, Offset(rrect.left, blY), Offset(rrect.left, tlY),
          dashPattern, paint);
    }
  }
